      ...
      <div className="card h-[500px] mb-3 bg-base-100 w-96 shadow-sm">
        ...
        <div className="card-body">
          <h2 className="card-title text-xl font-bold">
            {post.title}
            {
              post.availability === 'Available' ? <div className="badge text-white py-4 bg-purple-500"><BadgeCheck color="white" />Available</div> :
                <div className="badge text-white py-4 bg-purple-500"><BadgeX color="white" />Unavailable</div>
            }

          </h2>
          <h3 className='text-lg font-medium text-slate-500 mt-3'>Rent: {post.rent} Taka</h3>
          <div className='flex justify-between font-medium text-lg text-slate-500'>
            <h4>Location: {post.location}</h4>
            <h4>Roomtype: {post.roomType}</h4>
          </div>
